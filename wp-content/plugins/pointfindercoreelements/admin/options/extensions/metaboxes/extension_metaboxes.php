    ...
    if ( ! class_exists( 'ReduxFramework_extension_metaboxes' ) ) {
        ...
        class ReduxFramework_extension_metaboxes {
            ...
            function url_to_postid( $url ) {
                ...
                if ( false !== strpos( esc_url(home_url("/")), '://www.' ) && false === strpos( $url, '://www.' ) ) {
                    $url = str_replace( '://', '://www.', $url );
                }

                // Strip 'www.' if it is present and shouldn't be
                if ( false === strpos( esc_url(home_url("/")), '://www.' ) ) {
                    $url = str_replace( '://www.', '://', $url );
                }
                ...
                if ( false !== strpos( $url, esc_url(home_url("/")) ) ) {
                    // Chop off http://domain.com
                    $url = str_replace( esc_url(home_url("/")), '', $url );
                } else {
                    // Chop off /path/to/blog
                    $home_path = parse_url( esc_url(home_url("/")) );
                        ...
                        foreach ( $box['post_types'] as $posttype ) {
                            if ( isset( $box['title'] ) ) {
                                ...
                                if ( isset( $box['sections'] ) && count( $box['sections'] ) == 1 && isset( $box['sections'][0]['fields'] ) && count( $box['sections'][0]['fields'] ) == 1 && isset( $box['sections'][0]['fields'][0]['title'] ) ) {
                                    ...
                                    $title = ucfirst( $posttype ) . " " . esc_html__( 'Options', 'pointfindercoreelements' );
                                }
            ...
            function generate_boxes( $post, $metabox ) {
                ...
                <div
                    class="redux-container<?php echo ( esc_attr($sidebar) ) ? ' redux-has-sections' : ' redux-no-sections'; ?> redux-box-<?php echo $metabox['args']['position']; ?>">
                    <div class="redux-notices">
                        <?php if ( $sidebar ){ ?>
                            <div class="saved_notice admin-notice notice-blue" style="display:none;">
                                <strong><?php echo apply_filters( "redux-imported-text-{$this->parent->args['opt_name']}", esc_html__( 'Settings Imported!', 'pointfindercoreelements' ) ) ?></strong>
                            </div>
                            <div class="redux-save-warn notice-yellow">
                                <strong><?php echo apply_filters( "redux-changed-text-{$this->parent->args['opt_name']}", esc_html__( 'Settings have changed, you should save them!', 'pointfindercoreelements' ) ) ?></strong>
                            </div>
                        <?php }; ?>
                        <div class="redux-field-errors notice-red">
                            <strong><span></span> <?php echo esc_html__( 'error(s) were found!', 'pointfindercoreelements' ) ?></strong>
                        </div>
                        <div class="redux-field-warnings notice-yellow">
                            <strong><span></span> <?php echo esc_html__( 'warning(s) were found!', 'pointfindercoreelements' ) ?>
                            </strong></div>
                    </div>
                    <?php
                        echo '<a href="javascript:void(0);" class="expand_options hide" style="display:none;">' . esc_html__( 'Expand', 'pointfindercoreelements' ) . '</a>';
                        if ( $sidebar ) {
                            ?>
...
//print_r($this->options[$field['id']]);
            ...
            function meta_boxes_show_errors() {
                if ( isset( $this->notices['errors'] ) && ! empty( $this->notices['errors'] ) ) {
                    ...
                    echo '<p><strong><span></span> ' . count( $this->notices['errors'] ) . ' ' . esc_html__( 'error(s) were found!', 'pointfindercoreelements' ) . '</strong></p>';
                    echo '</div>';
                }
                if ( isset( $this->notices['warnings'] ) && ! empty( $this->notices['warnings'] ) ) {
                    ...
                    echo '<p><strong><span></span> ' . count( $this->notices['warnings'] ) . ' ' . esc_html__( 'warnings(s) were found!', 'pointfindercoreelements' ) . '</strong></p>';
                    echo '</div>';
                }
